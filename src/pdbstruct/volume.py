...
usage = """
Calculates the volume of atoms in a PDB/CIF file using bitarray-based grid method.

Usage: hollow-vol input_file [spacing] [chain] [residue_num]

Arguments:
    input_file    PDB or CIF file to analyze
    spacing       Grid spacing in Angstroms (default: 0.5)
    chain         Calculate volume for specific chain only (optional)
    residue_num   Calculate volume for specific residue number in chain (optional)

Examples:
    hollow-vol protein.pdb                    # Calculate total volume
    hollow-vol protein.pdb 0.3               # Higher resolution
    hollow-vol protein.pdb 0.5 A             # Chain A only
    hollow-vol protein.pdb 0.5 A 100         # Residue 100 in chain A
"""


def main():
    """
    Parse command line arguments for volume calculation.

    Returns:
        Tuple of (input_file, spacing, target_chain, target_res_num)
    """
    ...
    if len(sys.argv) > 2:
        try:
            ...
            print(f"Error: Invalid spacing value '{sys.argv[2]}'. Must be a number.")
            sys.exit(1)
    ...
    if len(sys.argv) > 4:
        try:
            target_res_num = int(sys.argv[4])
        except ValueError:
            print(f"Error: Invalid residue number '{sys.argv[4]}'. Must be an integer.")
            sys.exit(1)

    calc_volume(input_file, spacing, target_chain, target_res_num)
